...
!uv pip install openpipe-art==0.5.9 langchain-core tenacity datasets
...
print(
    f"Database contains the complete email dataset, loaded {len(training_scenarios)} training scenarios and {len(validation_scenarios)} validation scenarios."
)
...
# Decrese the number of turns from 10 to 6 to speed up training
MAX_TURNS = 6
    ...
    if batch.step % training_config["validation_step_interval"] == 0:
        ...
        for scenario in validation_scenarios:
            validation_groups.append(
                art.TrajectoryGroup([rollout(model, EmailScenario(step=batch.step, scenario=scenario))])
            )
